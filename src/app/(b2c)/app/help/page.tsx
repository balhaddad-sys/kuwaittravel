      ...
      <Container size="md" className="py-4 sm:py-6 space-y-4">
        <Card variant="elevated" padding="lg">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-body-lg sm:text-heading-sm font-bold text-stone-900 dark:text-white">{t("الأسئلة الشائعة", "FAQ")}</h2>
          </div>
          <div>
            {faqs.map((faq, i) => (
              <div key={i} className="border-b border-surface-border/60 last:border-0 dark:border-surface-dark-border/60">
                <button onClick={() => setOpenIdx(openIdx === i ? null : i)} className="flex w-full items-center justify-between gap-3 py-3.5 text-start">
                  <span className="text-body-sm sm:text-body-md font-semibold text-stone-900 dark:text-white">{t(faq.qAr, faq.qEn)}</span>
                  <ChevronDown className={cn("h-4 w-4 shrink-0 text-stone-400 transition-transform", openIdx === i && "rotate-180")} />
                </button>
                {openIdx === i && <p className="pb-3.5 text-body-sm text-stone-600 dark:text-stone-400 leading-relaxed">{t(faq.aAr, faq.aEn)}</p>}
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card variant="elevated" padding="lg">
          <h2 className="text-body-lg sm:text-heading-sm font-bold text-stone-900 dark:text-white mb-4">{t("تواصل معنا", "Contact Us")}</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-3 text-body-sm text-stone-700 dark:text-stone-300">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-stone-100 dark:bg-slate-700"><c.icon className="h-4 w-4 text-stone-500" /></span>
                <div>
                  <p className="font-medium">{t(c.labelAr, c.labelEn)}</p>
                  <p className="text-stone-500" dir="ltr">{c.value}</p>
                </div>
              </div>
            ))}
          </div>
