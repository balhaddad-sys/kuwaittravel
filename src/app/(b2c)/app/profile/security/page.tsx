    ...
    <div className="min-h-screen">
      ...
      <Container size="md" className="py-4 sm:py-6 space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3 rounded-lg border border-surface-border/70 p-3 dark:border-surface-dark-border/70">
              <div className="flex items-center gap-3">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 dark:bg-neutral-700/50"><Phone className="h-4 w-4 text-slate-500" /></span>
                <div>
            ...
            <div className="flex items-center justify-between gap-3 rounded-lg border border-surface-border/70 p-3 dark:border-surface-dark-border/70">
              <div className="flex items-center gap-3">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 dark:bg-neutral-700/50"><Mail className="h-4 w-4 text-slate-500" /></span>
                <div>
        ...
        <Card variant="elevated" padding="lg">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between"><span className="text-body-sm text-slate-600 dark:text-neutral-400">{t("التحقق", "Verification")}</span><Badge variant={userData?.isVerified ? "success" : "warning"} size="sm">{userData?.isVerified ? t("موثق", "Verified") : t("غير موثق", "Unverified")}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-body-sm text-slate-600 dark:text-neutral-400">{t("نوع الحساب", "Type")}</span><Badge variant="info" size="sm">{userData?.role === "campaign_owner" ? t("صاحب حملة", "Owner") : t("مسافر", "Traveler")}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-body-sm text-slate-600 dark:text-neutral-400">{t("الحالة", "Status")}</span><Badge variant="success" size="sm">{t("نشط", "Active")}</Badge></div>
          </div>
        </Card>
