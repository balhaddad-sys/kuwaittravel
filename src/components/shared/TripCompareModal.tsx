    ...
    <Modal open={open} onClose={onClose} title={t("مقارنة الرحلات", "Compare Trips")} size="xl">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[500px]">
          <thead>
            <tr>
              <th className="p-3 text-start text-body-sm font-medium text-stone-500 dark:text-stone-400" />
              {trips.map((trip) => (
                <th key={trip.id} className="p-3 text-center">
                  <p className="text-body-md font-bold text-stone-900 dark:text-white">{trip.title}</p>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className={i % 2 === 0 ? "bg-stone-50/50 dark:bg-stone-800/30" : ""}>
                <td className="p-3">
                  <span className="flex items-center gap-2 text-body-sm font-medium text-stone-600 dark:text-stone-300">
                    {row.icon}
                ...
                {trips.map((trip) => {
                  ...
                  return (
                    <td key={trip.id} className="p-3 text-center">
                      <span className={isBest ? "font-bold text-teal-600 dark:text-teal-400" : "text-stone-800 dark:text-stone-200"}>
                        {value}
                      </span>
                    </td>
                  );
