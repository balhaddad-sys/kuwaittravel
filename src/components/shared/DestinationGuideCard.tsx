  ...
  return (
    <Card variant="elevated" padding="none" hoverable onClick={onClick} className={cn("group", className)}>
      ...
      <div className="p-3">
        {highlights.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {highlights.slice(0, 3).map((h, i) => (
              <span key={i} className="inline-flex items-center gap-1 rounded-full bg-sky-50 px-2 py-0.5 text-[10px] font-medium text-sky-700 dark:bg-sky-900/30 dark:text-sky-300">
                <span>{h.icon}</span>
        ...
        <div className="mt-2 flex items-center gap-1 text-body-sm font-medium text-sky-600 dark:text-sky-400">
