...
function LanguageToggle({ className }: LanguageToggleProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex items-center gap-2 rounded-full border border-surface-border/80 bg-white/75 px-3 py-2 text-body-sm text-navy-700 backdrop-blur-sm transform-gpu transition-[transform,background-color,border-color,color,box-shadow] duration-[var(--duration-ui)] ease-[var(--ease-smooth)] hover:border-gold-300/70 hover:bg-white/92 active:scale-[0.97] dark:border-surface-dark-border/80 dark:bg-surface-dark-card/75 dark:text-navy-200 dark:hover:border-gold-700/60 dark:hover:bg-surface-dark-card",
        className
      )}
    >
