...
function AppBar({
  ...
  return (
    <header
      className={cn(
        "relative sticky top-0 z-[var(--z-topbar)] flex h-14 sm:h-[72px] items-center justify-between gap-3 sm:gap-4 border-b border-surface-border/80 dark:border-surface-dark-border/80 bg-white/78 dark:bg-surface-dark/70 backdrop-blur-sm px-4 sm:px-6 shadow-topbar",
        className
      )}
    >
      ...
      <div className="flex min-w-0 items-center gap-2 sm:gap-3">
        {onMenuToggle && (
          <button
            ...
            className="shrink-0 rounded-lg p-2 text-navy-400 transition-[background-color,color,transform] duration-[var(--duration-ui)] ease-[var(--ease-smooth)] hover:bg-surface-muted dark:hover:bg-surface-dark-card active:scale-[0.97] lg:hidden"
          >
